...
class NestPmc(Problem):
    ...
    def filter(self, node):
        ...
        if len(node.minor_vertices) > 0 and len(node.all_vertices) - len(node.vertices) <= self.inner_vars_threshold:
            ...
            candidate_tabs = ",".join(
                ["{} {}".format(var2tab(node, v), var2tab_alias(node, v)) for v in minor_vertices])
        ...
        for r in db.select_all(f"td_node_{node.id}", cols):
            if not self.interrupted:
                if len(node.all_vertices) - len(
                        node.vertices) > self.inner_vars_threshold:  # only if there is an inner problem to solve
                    futures.append(executor.submit(self.solve_sat, node, db, cols, r, rules))
        for future in as_completed(futures):
            if future.exception():
                raise future.exception()
        executor.shutdown(wait=True)

    def solve_sat(self, node, db, cols, vals, covered_rules):
        if self.interrupted:
            return
        try:
            where = []
